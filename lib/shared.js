...
(function(MU) {
    ...
    MU.SITE_CONFIG = {
        ...
        'mangalib.org':    { endpoint: 'manga', siteId: '1', name: 'MangaLib' },
        ...
        return new Promise((resolve, reject) => {
            chrome.runtime.sendMessage({
                ...
                if (chrome.runtime.lastError) {
                    // Сетевая ошибка / расширение не доступно
                    const err = new Error(chrome.runtime.lastError.message || 'runtime error');
                ...
                } else if (response && typeof response.status === 'number') {
                    // Сервер вернул HTTP-ответ, но статус не 2xx — это не сетевая ошибка
                    const err = new Error(`HTTP ${response.status}${response.text ? `: ${String(response.text).substring(0, 200)}` : ''}`);
                    err.status       = response.status;
                    err.responseText = response.text || '';
                    err.networkError = false;
                    reject(err);
                } else {
                    // Сетевая ошибка / fetch упал до получения ответа
                    const err = new Error(response?.error || 'fetch failed');
                    err.networkError = true;
                    reject(err);
                }
            });
